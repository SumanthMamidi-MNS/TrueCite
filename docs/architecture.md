...
- **Hosting**: GitHub itself is not a place this can run — it needs an actual Python/FastAPI-capable host with roughly 3–4 GB of RAM (the bge-m3 embedding model plus PyTorch), which rules out 512 MB free tiers such as Render's. Chosen target: any Docker host, run with `docker-compose.yml` (app + Caddy for automatic HTTPS); an Oracle Cloud Always Free ARM VM fits. Hugging Face Spaces was tried first and dropped over its one-running-free-Space quota.
